...
3. Spawn `agents/reviewer-issue.md` (Opus) via `skills/spawn-adversarial-review.md`. A FAIL is fixed, never overridden. Re-review with a fresh instance.
4. Spawn `agents/implementation-agent.md` (Sonnet) with the passing issue and all prior-art paths.
5. Spawn the appropriate `agents/reviewer-*.md` (Opus) against the artifact. Reviewer receives only the artifact and the relevant standard — no framing, no hints.
