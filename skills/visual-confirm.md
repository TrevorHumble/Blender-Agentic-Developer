...
description: >
  ...
  Trevor can SEE the result and confirm it is what he meant. Triggers: "render a
  preview", "let Trevor see it", "visual gate", "does this look right", any
...
prove the result is what Trevor *meant* — he is the final eye on the visual
result, and he cannot read code. This gate closes that gap: it renders the
add-on's output to a picture he can look at and say yes / no to.
...
produces a picture and a plain-language question; Trevor's answer is the signal.
...
## How the result is surfaced to Trevor

Show him the image with a plain-language, non-developer question. No jargon, no
code. Use this shape:
...
If he says **confirm**, the visible result is accepted; note it and move on.
If he says **it's wrong**, that is a real defect even though every math gate
passed — capture it as an issue and route it through the normal pipeline. His
eye is the authority here; do not argue the geometry is "correct" at him.
