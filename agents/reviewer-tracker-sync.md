...
**Input:** the repo root. Read `issues/` (the issue files and `0009-future-work-backlog.md`), `BUILDLOG.md`,
and the live board via `& "C:\Program Files\GitHub CLI\gh.exe" issue list --repo TrevorHumble/Blender-Agentic-Developer --state all --json number,title,state,labels`. Read nothing else.
...
- [ ] An issue file `issues/NNNN-*.md` has no matching GitHub card at all (missing from the board). Exception: `issues/0009-future-work-backlog.md` is the backlog *container*, not a task — it has no card; its actionable items (B-items, ECs) are each their own card instead.
